...
LOGVIEWER_PREFIX = "ProjectYaML::LogViewer::"
...
module ProjectYaML
  class Slice
    ...
    # ProjectYaML Slice Log
    # Used for log viewing
    class Log < ProjectYaML::Slice
      ...
      # Initializes ProjectYaML::Slice::Log including #slice_commands, #slice_commands_help
      # @param [Array] args
      def initialize(args)
        ...
        @logfile = File.join(get_logfile_path, "project_yaml.log")
      end
      ...
      # uses the location of the YaML configuration file to determine the path to the
      # ${yaml_HOME}/log directory (which is where the logfiles for YaML are located)
      ...
      def get_yaml_log
        ...
        raise ProjectYaML::Error::Slice::NotImplemented,
        ...
        unless /^[-]{1,2}.*$/.match(@command_array.first)
          raise ProjectYaML::Error::Slice::SliceCommandParsingFailed,
                "Unexpected argument found while parsing log slice command (#{@command_array.first})"
        end

        # load the appropriate option items for the subcommand we are handling
        option_items = command_option_data(:get_yaml_log)
        # parse and validate the options that were passed in as part of this
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        tmp, options = parse_and_validate_options(option_items, "yaml log (options...)", :require_all)
        ...
        if filter_options.length > 0
          ...
          if num_lines_tail && filter_before_tail
            ...
            filter_then_tail_yaml_log(filter_options, num_lines_tail)
      ...
      private
      # Prints the contents from the current yaml logfile to the command line
      def view_yaml_log
        ...
        begin
          ...
          raise ProjectYaML::Error::Slice::InternalError, "error while reading log file #{@logfile}; #{e.message}"
        end

      end

      # Prints the tail of the current yaml logfile to the command line
      def tail_yaml_log(num_lines_tail)
      ...
      # filters the current yaml logfile, printing all matching lines
      def filter_yaml_log(filter_options)
        ...
        begin
          ...
          raise ProjectYaML::Error::Slice::InternalError, "error while filtering log file #{@logfile}; #{e.message}"
        end

      end

      # tails the current yaml logfile, then filters the result
      def tail_then_filter_yaml_log(filter_options, num_lines_tail)
        ...
        begin
          ...
          raise ProjectYaML::Error::Slice::InternalError, "error while tailing, then filtering log file #{@logfile}; #{e.message}"
        end

      end

      # filters the current yaml logfile, then tails the result
      def filter_then_tail_yaml_log(filter_options, num_lines_tail)
        ...
        begin
          ...
          raise ProjectYaML::Error::Slice::InternalError, "error while filtering, then tailing log file #{@logfile}; #{e.message}"
        end
        tail_of_file.each { |line|
          puts line
        }
