...
module ProjectYaML
  module Persist
    # Persistence Controller for ProjectYaML
    class Controller
      ...
      def initialize()
        ...
        @config = ProjectYaML.config

        # init correct database object
        if (config.persist_mode == :mongo)
          logger.debug "Using Mongo plugin"
          require "project_yaml/persist/mongo_plugin" unless ProjectYaML::Persist.const_defined?(:MongoPlugin)
          @database = ProjectYaML::Persist::MongoPlugin.new
        elsif (config.persist_mode == :postgres)
          logger.debug "Using Postgres plugin"
          require "project_yaml/persist/postgres_plugin" unless ProjectYaML::Persist.const_defined?(:PostgresPlugin)
          @database = ProjectYaML::Persist::PostgresPlugin.new
        elsif (config.persist_mode == :memory)
          logger.debug "Using in-memory plugin"
          require "project_yaml/persist/memory_plugin" unless ProjectYaML::Persist.const_defined?(:MemoryPlugin)
          @database = ProjectYaML::Persist::MemoryPlugin.new
        else
      ...
      # Connect to database using ProjectYaML::Persist::Database::Plugin loaded
