...
module ProjectYaML
  module Persist
    # MongoDB version of {ProjectYaML::Persist::PluginInterface}
    # used by {ProjectYaML::Persist::Controller} when ':mongo' is the 'persist_mode' in
    ...
    class MongoPlugin < PluginInterface
      ...
      def connect(hostname, port, username, password, timeout)
        ...
        @yaml_database = @connection.db("project_yaml")
        @connection.active?
      end
      ...
      # Checks whether DB 'ProjectYaML' is selected in MongoDB
      #
      # @return [Boolean] Connection status
      #
      def is_db_selected?
        #logger.debug "Is ProjectYaML DB selected?(#{(@yaml_database != nil and @connection.active?)})"
        (@yaml_database != nil and @connection.active?)
      end
      ...
      def collection_by_name(collection_name)
        if is_db_selected?
          @yaml_database.collection(collection_name.to_s)
        else
