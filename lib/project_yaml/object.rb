...
# Common object for all base ProjectYaML objects
# @abstract
class ProjectYaML::Object
  # Mixin our ObjectUtilities
  include(ProjectYaML::Utility)
  include(ProjectYaML::Logging)

  # There variables are required in all ProjectYaML objects
  ...
  attr_accessor :_persist_ctrl # instance ref pointing to ProjectYaML::Persist::Controller of ProjectYaML::Data that created/fetched this object used for update/refresh
  attr_accessor :_namespace # Collection/Table symbol for ProjectYaML::Persist::Controller / Must be specified(overridden) in each child class
  # TODO - Change is_template to be a method from any object returning the template details
  attr_accessor :is_template # For objects with a template -> instance mapping. This flag is set to false when object is an instance
  attr_reader   :noun # Noun representing object for API representation
  ...
  def refresh_self
    ...
    new_hash = @_persist_ctrl.object_hash_get_by_uuid(self.to_hash, @_namespace)
  ...
  def get_data
    data = ProjectYaML::Data.instance
    data.check_init
    data
  end
