...
class ProjectYaML::Slice < ProjectYaML::Object
  ...
  def initialize(args)
    ...
    @prev_args = Stack.new
    @hidden = true
    @uri_root = ProjectYaML.config.yaml_uri + "/yaml/api/"
    @data = get_data
  end
  ...
  def eval_command(command_hash)
    unless @command_array.count > 0
      ...
      if command_hash[:default]
        ...
        raise ProjectYaML::Error::Slice::Generic, "No Default Action"
      end
    end
    ...
    if command_hash[:else]
      ...
      raise ProjectYaML::Error::Slice::InvalidCommand, "System Error: no else action for slice"
    end
  end
  ...
  def slice_success(response, options = {})
    ...
    return_hash["client_config"] = ProjectYaML.config.get_client_config_hash if mk_response
    if @web_command
  ...
  def slice_error(error, options = {})
    ...
    return_hash = {}
    log_level = :error
    if error.class.ancestors.include?(ProjectYaML::Error::Slice::Generic)
  ...
  def command_option_data(command)
    ...
    # cares, since nothing in YaML catches it specifically. --daniel 2013-04-16
  ...
  def get_options(options = { }, optparse_options = { })
    optparse_options[:banner] ||= "yaml [command] [options...]"
  ...
  def validate_options(validate_options = { })
    ...
    when :require_one
      ...
      raise ProjectYaML::Error::Slice::MissingArgument, "Must provide one option from #{option_names.inspect}." if count < 1
    when :require_all
      validate_options[:option_items].each do
        |opt_item|
        raise ProjectYaML::Error::Slice::MissingArgument, "Must Provide: [#{opt_item[:description]}]" if opt_item[:required] && !validate_arg(validate_options[:options][opt_item[:name]])
      end
    end
  ...
  def print_command_help(command, option_items, contained_resource = nil)
    ...
    if contained_resource
      banner = ( option_items.select { |elem| elem[:uuid_is] == "required" }.length > 0 ?
        "yaml #{slice_name} (#{slice_name.upcase}_UUID) #{contained_resource} #{command} (UUID) (options...)" :
        "yaml #{slice_name} (#{slice_name.upcase}_UUID) #{contained_resource} #{command} (options...)")
    else
      banner = ( option_items.select { |elem| elem[:uuid_is] == "required" }.length > 0 ?
        "yaml #{slice_name} #{command} (UUID) (options...)" :
        "yaml #{slice_name} #{command} (options...)")
    end
  ...
  def throw_syntax_error
    command_str = "yaml #{slice_name} #{@prev_args.join(" ")}"
  ...
  def throw_missing_uuid_error
    raise ProjectYaML::Error::Slice::MissingArgument,
